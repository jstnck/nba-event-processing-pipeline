...
object StreamingJob extends App with SparkSessionWrapper {
  ...
  val schema = spark.read.json("file:///jars/frame_data.json").schema
  val kafkaReaderConfig = KafkaReaderConfig("kafka:9092", "dbserver1.wcd.frame_data")
  val jdbcConfig = JDBCConfig(url = "jdbc:postgresql://localhost:5432/test")
  new StreamingJobExecutor(spark, kafkaReaderConfig, currentDirectory + "/checkpoint/job", jdbcConfig, schema).execute()
}
...
class StreamingJobExecutor(spark: SparkSession, kafkaReaderConfig: KafkaReaderConfig, checkpointLocation: String, jdbcConfig: JDBCConfig, schema: StructType) {
